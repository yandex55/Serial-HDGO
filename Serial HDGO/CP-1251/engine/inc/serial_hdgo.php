...
if($action == "save")
{
	...
	if($cat_ex)
	{
		foreach($cat_ex as $index => $val)
		{
			$cat_ex[$index] = intval($val);
		}
	...
	if($cat_all)
	{
		foreach($cat_all as $index => $val)
		{
			$cat_all[$index] = intval($val);
		}
		$cat_all = implode(",", $cat_all);
	}
	
	$cat_noall = $_POST["cat_noall"];

	if($cat_noall)
	{
		foreach($cat_noall as $index => $val)
		{
			$cat_noall[$index] = intval($val);
		}
		$cat_noall = implode(",", $cat_noall);
	}
	...
	fwrite($handler, "'cat_ex' => '{$cat_ex}',\n\n");
	fwrite($handler, "'cat_all' => '{$cat_all}',\n\n");
	fwrite($handler, "'cat_noall' => '{$cat_noall}',\n\n");
...
$cat_ex = CategoryNewsSelection((empty($serial_hdgo['cat_ex']) ? 0 : explode(",",$serial_hdgo['cat_ex'])));
$cat_all = CategoryNewsSelection((empty($serial_hdgo['cat_all']) ? 0 : explode(",",$serial_hdgo['cat_all'])));
$cat_noall = CategoryNewsSelection((empty($serial_hdgo['cat_noall']) ? 0 : explode(",",$serial_hdgo['cat_noall'])));
...
HTML;
	...
	showRow("Исключить Категории", "Выберите все категории которые нужно исключить (внимание, если новость находится одновременно в категории которую нужно исключить и которая нужна, то модуль не сработает)", showSelect("cat_ex[]", $cat_ex, false));
	showRow("Категории для не посезоного вывода", "Выберите все категории в которых модуль будет работать как не посезонный вывод (работает только если включен посезонный вывод. <b style=\"color:red\">Заполнять нужно только в том случае если у вас есть категории в которых нужен не посезонный вывод! В противном случае оставьте поле пустым!</b>)", showSelect("cat_all[]", $cat_all, false));
	showRow("Категории для посезоного вывода", "Выберите все категории в которых модуль будет работать как посезонный вывод (работает только если выключен посезонный вывод. <b style=\"color:red\">Заполнять нужно только в том случае если у вас есть категории в которых нужен посезонный вывод! В противном случае оставьте поле пустым!</b>)", showSelect("cat_noall[]", $cat_noall, false));
	...
	showRow("Номер сезона", "Выберите доп поле в котором находится номер сезона (Только число. <b style=\"color:red\">Данное поле нужно только для посезоного вывода!</b>)", showSelect("save_con[season]", makeDropDown($xf_select, (empty($serial_hdgo['season']) ? false : $serial_hdgo['season'])), true));
	showRow("Номер серии", "Выберите доп поле в котором находится номер серии (Только число. <b style=\"color:red\">Данное поле нужно как для посезоного вывода так и не посезоного!</b>)", showSelect("save_con[seria]", makeDropDown($xf_select, (empty($serial_hdgo['seria']) ? false : $serial_hdgo['seria'])), true));
	showRow("Вывод первой серии", "<b style=\"color:red\">Вместо последней найбольшей серии будет выбиратся первая серия для посетителя</b>", makeCheckBox( "save_con[min_player_seria]", "{$serial_hdgo['min_player_seria']}" ));
	...
	showRow("Подписки", "Включить интеграцию с https://quantum-soft.net/dle/modules/49095-podpiska-na-obnovlenie-novostey-dlya-dle.html? (Только если он у вас есть)", makeCheckBox( "save_con[podpiska]", "{$serial_hdgo['podpiska']}" ));
	showRow("API TOKEN", "Введите свой api token с HDGO", showInput("save_con[token]", $serial_hdgo['token']));
echo <<<HTML
				</table>
			</div>
...
HTML;
	...
	showRow("Одна озвучка", "Выводить в блок только по одной озвучке, у которой найбольше серий?", makeCheckBox( "save_con[block_one]", "{$serial_hdgo['block_one']}" ));
echo <<<HTML
				</table>
			</div>
			<div class="box-footer padded">
	...
	<div id="data" style="display:none;">
		<div class="box">
			...
			<div class="box-content">
				<table class="table table-normal">
HTML;
	showRow("Вывод сезона", "Выберите доп поле в которое записывать сезон (<b style=\"color:red\">Доп поле должно быть другим чем в параметре Номер сезона</b>)", showSelect("save_con[season_in]", makeDropDown($xf_select, (empty($serial_hdgo['season_in']) ? false : $serial_hdgo['season_in'])), true));
	showRow("Вывод серии", "Выберите доп поле в которое записывать серию (<b style=\"color:red\">Доп поле должно быть другим чем в параметре Номер серии</b>)", showSelect("save_con[seria_in]", makeDropDown($xf_select, (empty($serial_hdgo['seria_in']) ? false : $serial_hdgo['seria_in'])), true));
	...
	showRow("Закончен сериал", "Выберите доп поле которое отвечает за то что закончен сериал или нет. Нужно для того что бы модуль больше не слал запросы а так же сформировал Meta Title для завершенного сериала.", showSelect("save_con[end_serial]", makeDropDown($xf_select, (empty($serial_hdgo['end_serial']) ? false : $serial_hdgo['end_serial'])), true));
	...
	showRow("Закончен сериал (количество серий)", "Выберите доп поле которое содержит в себе окончательное количество серий сериала (альтернатива двум верхним пунктам на проверку сериала. <b style=\"color:red\">Если вы выбрали параметр выше, то этот выбирать не нужно. Только один!</b>).", showSelect("save_con[end_serial_seria]", makeDropDown($xf_select, (empty($serial_hdgo['end_serial_seria']) ? false : $serial_hdgo['end_serial_seria'])), true));
	showRow("Вывод плеера", "Оформленный блок с запоминанием серии, сезона и озвучки", makeCheckBox( "save_con[player]", "{$serial_hdgo['player']}" ));
	showRow("Вывод ссылки на видео", "<b style=\"color:red\">Вместо блока будет только iframe с видео</b>", makeCheckBox( "save_con[design]", "{$serial_hdgo['design']}" ));
echo <<<HTML
				</table>
			</div>
			<div class="box-footer padded">
