...
{
	$sql_count = $db->super_query("SELECT COUNT(*) as count FROM ". PREFIX . "_serial_hdgo as s, " . PREFIX . "_post as p WHERE s.news_id=p.id $where_date AND approve=1");
	...
	$sql_serial_hdgo = $db->query("SELECT s.id as sid, p.id as pid, voice, season, seria, date_update, news_id, title, category, date, alt_name, short_story, xfields, end_serial_hdgo FROM " . PREFIX . "_serial_hdgo as s, " . PREFIX . "_post as p WHERE s.news_id=p.id AND approve=1 ORDER BY date_update DESC LIMIT {$cstart},{$serial_hdgo["limit_news"]}");
	$news_count = $cstart + $serial_hdgo["limit_news"];
}
else
{
	$sql_serial_hdgo = $db->query("SELECT s.id as sid, p.id as pid, voice, season, seria, date_update, news_id, title, category, date, alt_name, short_story, xfields, end_serial_hdgo FROM " . PREFIX . "_serial_hdgo as s, " . PREFIX . "_post as p WHERE s.news_id=p.id AND date_update >= DATE_SUB(CURRENT_DATE, INTERVAL {$serial_hdgo["interval_date"]} DAY) AND approve=1 ORDER BY date_update DESC LIMIT 0,{$serial_hdgo["limit_news"]}");
}
$news_id_array = array();
